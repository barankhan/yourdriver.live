...
class User extends  baseModel implements JsonSerializable {
    ...
    public function getAvailableDrivers($lat,$lng,$limit=1,$radius=5){
        ...
        $q = "SELECT *, (6371*acos(cos(radians(:lat))*cos(radians(lat))*cos(radians(lng)-radians(:lng))
            + sin(radians(:lat))*sin(radians(lat)))) AS distance FROM users where is_driver=1 and is_driver_online=1 and is_driver_on_trip=0 HAVING distance < :radius ORDER BY distance
          LIMIT :limit;";
        $params = array("lat"=>$lat,"lng"=>$lng,"limit"=>$limit,"radius"=>$radius);
